...
`figures/grafer/enkel-graf.svg`: Tre små grafer ved siden av hverandre.

- Grafen med fem hjørner der hvert par av forskjellige hjørner er forbundet med en kant.
- Den bipartitte grafen med tre hjørner i hver del og alle mulige kanter mellom delene.
- En enkel plan graf som senere kan gjenbrukes i eksempler og figurer.

I den plane grafen merkes ett hjørne $v$, noen naboer til $v$, én kant $e=\{u,v\}$ og endepunktene $u,v$.
De to første grafene skal tegnes på samme måte som når de dukker opp igjen senere i kapitlet.
:::
...
::: {.callout-note title="Figurforslag (SVG)"}
`figures/grafer/delgraf.svg`: Bruk den plane grafen fra `enkel-graf.svg` og vis tre forskjellige delgrafer:

1. en delgraf der bare noen kanter er fjernet;
2. en delgraf der noen hjørner og alle kantene som støter til dem er fjernet;
3. en delgraf som ikke er av noen av disse to spesielle formene.

Behold de samme hjørneplasseringene som i originalgrafen.
:::
...
::: {.callout-note title="Figurforslag (SVG)"}
`figures/grafer/tre-skog-sykel.svg`: Tre små bilder:

- et tre;
- en skog med to komponenter;
- det samme treet som i første bilde, men med én ekstra kant slik at det oppstår en sykel.
:::
...
::: {.callout-note title="Figurforslag (SVG)"}
`figures/grafer/bipartitt.svg`: To bipartitte grafer med delene $A$ og $B$ tegnet som to kolonner.

- Den bipartitte grafen med tre hjørner i hver del og alle mulige kanter mellom delene, tegnet nøyaktig som i `enkel-graf.svg`.
- En annen bipartitt graf der ikke alle mulige kanter mellom delene er med.
:::
...
::: {.callout-note title="Figurforslag (SVG)"}
`figures/grafer/to-ikkeplane-grafer.svg`: Vis de to grafene fra @exr-to-ikkeplane med nøyaktig samme
hjørneplassering og utforming som i `enkel-graf.svg` og `bipartitt.svg`, slik at de er umiddelbart gjenkjennelige.
:::
...
::: {.callout-note title="Figurforslag (SVG)"}
`figures/grafer/subdivisjon.svg`:

- Vis først én kant $u$--$v$ og ved siden av samme kant subdividert med ett nytt hjørne $x$: $u$--$x$--$v$.
- Vis deretter en liten versjon av den første grafen i @exr-to-ikkeplane der én kant er subdividert.
:::
