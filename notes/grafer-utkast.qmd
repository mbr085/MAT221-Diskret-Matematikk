...
En liten enkel graf med 5--6 hjørner. Merk ett hjørne $v$, to naboer til $v$, en kant
$e=\{u,v\}$ og endepunktene $u,v$. Figuren bør stå rett etter den første grafdefinisjonen.
Foreslått filnavn: `figures/grafer/enkel-graf.svg`.
:::
...
::: {.callout-note title="Figurforslag (SVG)"}
Vis samme graf som i den første figuren, og ved siden av en delgraf der noen hjørner og kanter
er beholdt. Bruk samme plassering av hjørnene slik at forholdet er umiddelbart synlig.
Foreslått filnavn: `figures/grafer/delgraf.svg`.
:::
...
::: {.callout-note title="Figurforslag (SVG)"}
Én graf med tre tydelig markerte ruter: en gang som gjentar et hjørne, en sti og en sykel.
Helst samme underliggende graf i alle tre småpanelene.
Foreslått filnavn: `figures/grafer/gang-sti-sykel.svg`.
:::
...
::: {.callout-note title="Figurforslag (SVG)"}
Tre små bilder: et tre, en skog med to komponenter, og det samme treet med én ekstra kant
slik at det oppstår en sykel. Foreslått filnavn: `figures/grafer/tre-skog-sykel.svg`.
:::
...
La $G=(V,E)$ være en endelig, sammenhengende vektet enkel graf. Et **spenntre** i $G$ er en
delgraf
...
::: {.callout-note title="Figurforslag (SVG)"}
En liten vektet graf og samme graf ved siden av med ett spenntre markert. Alle hjørnene skal
stå på samme sted i begge bilder; bare de valgte kantene fremheves i spenntreet.
Foreslått filnavn: `figures/grafer/spenntre.svg`.
:::
...
- **Kruskal:** 4--5 små paneler der skogen vokser. Vis minst én kant som avvises fordi den ville danne en sykel.
- **Prim:** 4--5 små paneler fra et markert starthjørne der ett sammenhengende tre vokser.

Samme graf og samme vekter i begge serier gjør kontrasten tydelig.
Foreslåtte filnavn: `figures/grafer/kruskal-steg.svg` og `figures/grafer/prim-steg.svg`.
:::
...
::: {.callout-note title="Figurforslag (SVG)"}
Vis én og samme plane graf først i en tegning med en unødvendig kantkryssing og deretter i en
plan innleiring uten kryssinger. Poenget er å skille «denne tegningen har en kryssing» fra
«grafen er ikke plan». Foreslått filnavn: `figures/grafer/plan-tegning.svg`.
:::
...
::: {.callout-note title="Figurforslag (SVG)"}
En todelt figur: til venstre et enkelt kart/graf på sfæren med ett punkt $p$ i en region,
til høyre den tilsvarende plane tegningen der nettopp denne regionen er den ytre flaten.
Foreslått filnavn: `figures/grafer/sfaere-plan.svg`.
:::
...
::: {.callout-note title="Figurforslag (SVG)"}
En liten bipartitt graf med $A$ tegnet som én vertikal kolonne og $B$ som en annen.
Ingen kanter går mellom to hjørner i samme kolonne.
Foreslått filnavn: `figures/grafer/bipartitt.svg`.
:::
...
4. Anta nå at $G$ er bipartitt. Bruk @exr-bipartitt-sykler til å forklare hvorfor ingen flate kan ha rand av lengde tre.
...
### To grunnleggende hindringer for planaritet

::: {.callout-note title="Figurforslag (SVG)"}
To figurer side ved side:

1. fem hjørner der hvert par av forskjellige hjørner er forbundet med en kant;
2. to grupper med tre hjørner, der hvert hjørne i den ene gruppen er forbundet med alle tre i den andre.

Ingen symbolnavn er nødvendig i selve figuren. Foreslått filnavn:
`figures/grafer/to-ikkeplane-grafer.svg`.
:::
...
Å **subdividere** en kant betyr å erstatte den med en sti ved å sette inn ett eller flere nye
hjørner av grad $2$.

::: {.callout-note title="Figurforslag (SVG)"}
Vis én kant $u$--$v$ og ved siden av samme kant subdividert med ett nytt hjørne $x$:
$u$--$x$--$v$. Legg deretter inn en liten versjon av den første ikke-plane grafen der én kant
er subdividert. Foreslått filnavn: `figures/grafer/subdivisjon.svg`.
:::
...
::: {.callout-note title="Figurforslag (SVG)"}
En plan graf og dens dual oppå samme tegning, men visuelt tydelig atskilt. Deretter gjerne et lite
panel med kube-grafen og oktaeder-grafen som duale.
Foreslått filnavn: `figures/grafer/dual-graf.svg`.
:::
...
::: {.callout-note title="Figurforslag (SVG)"}
En svært enkel strekfigur av kube og oktaeder side ved side, eventuelt også tetraederet som selvdual.
Dodekaeder/ikosaeder kan utelates hvis figuren blir for tung.
Foreslått filnavn: `figures/grafer/polyeder-dualitet.svg`.
:::
...
::: {.callout-note title="Figurforslag (SVG)"}
Et lite kunstig kart med 5--6 regioner og den duale grafen ved siden av. Bruk ingen faktisk
geografi; en ren skjematisk figur er lettere å lese.
Foreslått filnavn: `figures/grafer/kart-og-dual.svg`.
:::
...
Hjørnene i enhver endelig enkel plan graf kan fargelegges med høyst fire farger slik at nabohjørner får
forskjellige farger.
:::
...
ethvert endelig kart på sfæren eller i planet kan fargelegges med høyst fire farger når regioner som
...
**graf**, **delgraf**, **sammenheng**, **tre**, **skog**, **spenntre**, **plan graf**, **sfære**, **flate**,
...
- bruke de grunnleggende definisjonene for grafer, delgrafer, stier, sykler, sammenheng og trær,
- forklare og anvende sentrale karakteriseringer av trær,
- forklare hva en vektet graf og et spenntre er,
